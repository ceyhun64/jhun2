...
// Proje Kartı İskeleti - Light/Dark Theme Uyumlu
const ProjectCardSkeleton = () => (
  <div className="w-full max-w-[400px]">
    <div className="bg-gray-100 dark:bg-zinc-950/50 border border-gray-300 dark:border-zinc-800 rounded-2xl p-2.5 shadow-lg text-left">
      <Skeleton className="relative aspect-video rounded-xl bg-gray-300 dark:bg-zinc-800" />
      <div className="p-1.5">
        <Skeleton className="mt-4 h-6 w-3/4 rounded bg-gray-300 dark:bg-zinc-800" />
        <Skeleton className="mt-3 h-4 w-full rounded bg-gray-300 dark:bg-zinc-800" />
        <Skeleton className="mt-2 h-4 w-11/12 rounded bg-gray-300 dark:bg-zinc-800" />
...
export default function ProjectsClient({ dict, locale }: Props) {
  ...
  // Başlık Alanı İçin Skeleton
  const renderHeaderSkeletons = () => (
    <div className="relative w-full flex flex-col justify-center items-center text-center z-10 mt-5">
      <Skeleton className="h-20 w-3/4 max-w-xl rounded-lg bg-gray-300 dark:bg-zinc-800 mb-4" />
      <Skeleton className="h-4 w-1/2 max-w-md rounded bg-gray-300 dark:bg-zinc-700" />
      <Skeleton className="h-4 w-2/3 max-w-md rounded bg-gray-300 dark:bg-zinc-700 mt-2 mb-6" />
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#F5F7FA] via-gray-200 to-[#F5F7FA] dark:from-black dark:via-amber-950/80 dark:to-black text-gray-900 dark:text-white py-15 md:py-20 px-4 md:px-10 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto text-center relative z-10 font-mono">
        ...
        {error && (
          <div className="text-red-500 dark:text-red-400 mt-10">{error}</div>
        )}
                  ...
                  {isMobile ? (
                    ...
                    >
                      <CardBody className="relative bg-gradient-to-b from-zinc-50/5 via-zinc-100/20 to-zinc-300/40 dark:from-zinc-950/5 dark:via-zinc-900/20 dark:to-zinc-800/50 border border-gray-800/10 dark:border-gray-200/10  rounded-2xl p-3 group/card hover:border-blue-500/40 transition-all duration-500 hover:shadow-[0_0_25px_rgba(59,130,246,0.25)] hover:z-10 text-left">
                        ...
                        <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-blue-500/10 via-orange-500/10 to-blue-500/10 dark:from-blue-500/20 dark:via-orange-500/20 dark:to-blue-500/20 opacity-0 group-hover/card:opacity-100 blur-[25px] transition-opacity duration-700"></div>
                      </CardBody>
                    </CardContainer>
                  )}
                </motion.div>
