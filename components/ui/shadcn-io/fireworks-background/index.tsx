...
// Gece gökyüzü tonlarında renk üret
const randColor = () => {
  const hues = [200, 220, 240, 260, 280, 300]; // mavi-mor tonları
  const hue = hues[randInt(0, hues.length)];
  const lightness = rand(40, 70);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        className,
        "bg-gradient-to-br from-sky-100 via-blue-50 to-green-100 dark:from-black dark:via-indigo-950 dark:to-black rounded-xl"
      )}
      {...props}
    >
      <canvas
        {...canvasProps}
        ref={canvasRef}
        className={cn("absolute inset-0 w-full h-full", canvasProps?.className)}
      />
    </div>
  );
}
